...
const OCPP_16_SUBPROTOCOLS = new Set([
  'ocpp1.6',
  'ocpp1.6j',
  'ocpp1.6-json',
  'ocpp1.6j-json',
...
const wss = new WebSocketServer({
  ...
  handleProtocols: selectOcppSubprotocol
});
...
function normalizeChargePointId(value) {
...
function parseOcppConnectionRequest(req) {
  ...
  const queryChargePointId = firstQueryValue(url.searchParams, [
    'chargePointId',
    'charge_point_id',
    'chargeBoxId',
    'chargeBoxIdentity',
  ...
  if (segments[0].toLowerCase() === 'ocpp') {
    ...
    return {
      valid: true,
      chargePointId: pathChargePointId || queryChargePointId,
      pendingIdentity: !(pathChargePointId || queryChargePointId)
    };
  }
  ...
  return { valid: false, reason: 'rota invalida' };
}
...
function createCharger(chargePointId) {
  return {
    id: makeId(),
    charge_point_id: chargePointId,
    ...
    connectors: [createDefaultConnector()]
  };
}

function getOrCreateChargerByChargePointId(chargePointId) {
  let charger = chargers.find(c => c.charge_point_id === chargePointId);
  if (!charger) {
    charger = createCharger(chargePointId);
...
function getOrCreateConnector(charger, connectorId) {
  ...
  if (!connector) {
    connector = { ...createDefaultConnector(), connector_number: number, status: 'Unknown' };
...
function normalizePowerToKw(value, unit) {
  ...
  if (Number.isNaN(numeric)) return null;
  const normalizedUnit = String(unit || '').trim().toLowerCase();
  if (normalizedUnit === 'w') return numeric / 1000;
  if (normalizedUnit === 'kw') return numeric;
  if (!unit && numeric > 1000) return numeric / 1000;
  return numeric;
}

function findSampledValue(sampledValues, measurands) {
  if (!Array.isArray(sampledValues)) return null;
  const normalizedMeasurands = measurands.map(measurand => measurand.toLowerCase());
  return sampledValues.find(sv => {
    const measurand = String(sv.measurand || '').toLowerCase();
    return normalizedMeasurands.includes(measurand) || normalizedMeasurands.some(expected => measurand.startsWith(expected));
  }) || null;
}
...
function getBootChargePointId(payload) {
...
function inferConnectorPowerKw(description) {
  if (/sol\s*7[,.]?4|sol7[,.]?4|7[,.]?4\s*kw/i.test(description)) return 7.4;
  if (/7[,.]?5\s*kw/i.test(description)) return 7.5;
  if (/11\s*kw|sol\s*11|sol11/i.test(description)) return 11;
  if (/22\s*kw|sol\s*22|sol22/i.test(description)) return 22;
  return null;
}

function applyBootMetadata(charger, payload) {
  ...
  const description = `${charger.fabricante || ''} ${charger.modelo || ''}`.toLowerCase();
  const looksLikeAcWallbox = /solplanet|aiswei|apollo|wallbox|wall box|\bac\b|sol7|sol11|sol22/.test(description);
  if (looksLikeAcWallbox) {
    const connector = getOrCreateConnector(charger, 1);
    if (!connector.type || connector.type === 'CCS2') connector.type = 'Type2';

    const inferredPowerKw = inferConnectorPowerKw(description);
    if (inferredPowerKw && (!connector.powerKw || Number(connector.powerKw) === 50)) {
      connector.powerKw = inferredPowerKw;
    }
  }
}

function rebindPendingConnectionIdentity(ws, currentChargePointId, newChargePointId, charger) {
  const existing = chargers.find(c => c.charge_point_id === newChargePointId);
  let reboundCharger = charger;

  if (existing && existing !== charger) {
    if (!existing.connectors || existing.connectors.length === 0) existing.connectors = charger.connectors;
    reboundCharger = existing;
  } else {
    charger.charge_point_id = newChargePointId;
  }

  chargers = chargers.filter(c => c === reboundCharger || c.charge_point_id !== currentChargePointId);
  if (activeConnections.get(currentChargePointId) === ws) activeConnections.delete(currentChargePointId);
  activeConnections.set(newChargePointId, ws);
  return reboundCharger;
}
  ...
  ws.on('message', async (message) => {
    ...
    try {
      if (Array.isArray(parsed) && parsed[0] === 2) {
        ...
        if (action === 'BootNotification' && pendingIdentity) {
          const bootChargePointId = getBootChargePointId(payload);
          if (bootChargePointId && bootChargePointId !== chargePointId) {
            const previousChargePointId = chargePointId;
            charger = rebindPendingConnectionIdentity(ws, chargePointId, bootChargePointId, charger);
            chargePointId = bootChargePointId;
            pendingIdentity = false;
        ...
        if (action === 'BootNotification') {
          ...
          const meterStartKwh = normalizeEnergyToKwh(payload.meterStart, 'Wh') || 0;
          ...
          if (session) {
            ...
            if (payload.meterStop !== undefined) {
              const finalMeterKwh = normalizeEnergyToKwh(payload.meterStop, 'Wh');
        ...
        } else if (action === 'DiagnosticsStatusNotification' || action === 'FirmwareStatusNotification') {
          responsePayload = {};
...
app.get('/', (req, res) => {
  res.json({
    ...
    websocketProtocols: Array.from(OCPP_16_SUBPROTOCOLS),
    timestamp: nowIso()
  });
});
